...
function AuthPageInner() {
  ...
  const handlePinLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Email sau PIN incorect')

      const { error } = await supabase.auth.verifyOtp({ type: 'email', token_hash: data.token_hash })
      if (error) throw error

      if (capsuleId) await fetch('/api/capsule/associate', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ capsule_id: capsuleId }) })
      router.push(redirectTo === '/profile' ? '/garderoba' : redirectTo)
    } catch (err: any) {
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-sm">
          ...
          <div className="text-center mb-8">
            <h1 className="font-display text-3xl font-light text-stone-900 mb-1">
              {mode==='login'?'Bine ai revenit':mode==='signup'?'Creează cont':'Intră cu PIN'}
            </h1>
            <p className="text-sm text-stone-500">
              {mode==='login'?'Accesează capsulele salvate.':mode==='signup'?'Salvează capsula și istoricul de comenzi.':'Email + PIN-ul de 4 cifre setat la prima accesare.'}
            </p>
          </div>
          ...
          <div className="space-y-3 mb-4">
            ...
            <input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&(mode==='pin'?handlePinLogin():handleEmailAuth())} className="w-full px-4 py-3 text-sm border border-stone-200 rounded-xl bg-white focus:outline-none focus:border-stone-500" />
            {mode==='pin'
              ? <input type="text" inputMode="numeric" autoComplete="off" maxLength={4} placeholder="PIN (4 cifre)" value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,'').slice(0,4))} onKeyDown={e=>e.key==='Enter'&&handlePinLogin()} className="w-full px-4 py-3 text-sm border border-stone-200 rounded-xl bg-white text-center tracking-[0.4em] focus:outline-none focus:border-stone-500" />
          ...
          {mode==='pin'
            ? <button onClick={handlePinLogin} disabled={loading||!email||pin.length<4}
                className={`w-full py-3.5 rounded-xl text-sm font-medium transition-all ${loading||!email||pin.length<4?'bg-stone-200 text-stone-400 cursor-not-allowed':'bg-stone-900 text-white hover:bg-stone-800 active:scale-[0.98]'}`}>
                {loading ? '...' : 'Intră'}
              </button>
          ...
          <button onClick={()=>{setMode(m=>m==='pin'?'login':'pin');setError('');setSuccess('')}} className="w-full mt-2 text-sm text-stone-400 hover:text-stone-700">
            {mode==='pin'?'← Intră cu email + parolă':'Ai primit o capsulă pe email? Intră cu PIN'}
          </button>
